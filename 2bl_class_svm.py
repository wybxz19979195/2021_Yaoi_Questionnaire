# -*- coding: utf-8 -*-
"""
Created on Mon Sep 27 17:37:40 2021
...
dt = pd.read_excel('1003_v3_3_distinguish_new2.xlsx')
data = pd.concat([dt[ (dt['143、您看耽美吗']==1)   & (dt['time_ln_z']> -1.5)] , 
                  dt[ (dt['143、您看耽美吗']==2)  & (dt['time_ln_z']> -1.5)]] ,axis = 0)

#converters={4:Iris_label}中“4”指的是第5列：将第5列的str转化为label(number)
#print(data.shape)
 
#2.划分数据与标签
#y,x=np.split(data.iloc[:,1:8],indices_or_sections=(0,),axis=1) #x为数据，y为标签
x = data.iloc[:,3:8]
y = data['143、您看耽美吗'].replace(2,0)
...
#5.绘制图形
#确定坐标轴范围
x1_min, x1_max=np.array(x)[:,0].min(), np.array(x)[:,0].max() #第0维特征的范围
x2_min, x2_max=np.array(x)[:,1].min(), np.array(x)[:,1].max() #第1维特征的范围
